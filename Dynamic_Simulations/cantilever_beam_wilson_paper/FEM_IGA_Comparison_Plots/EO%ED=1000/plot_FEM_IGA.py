...
"""# Process the IGA-FEM Partitioned data
with open ('tip_y_displacement_partitioned_iga_fem.json','r') as f_iga_fem_part:
    lines_iga_fem_part = f_iga_fem_part.readlines()

y_disp_iga_fem_part = []

for line in lines_iga_fem_part:
    split_iga_fem_part = line.split()
    if '"DISPLACEMENT_Y":' in split_iga_fem_part:
        continue
    if "{" in split_iga_fem_part:
        continue
    if "[" in split_iga_fem_part:
        continue
    if "]" in split_iga_fem_part:
    ...
    if "}" in split_iga_fem_part:
        continue
    y_disp_iga_fem_part.append(float(split_iga_fem_part[0].replace(',', '')))"""
